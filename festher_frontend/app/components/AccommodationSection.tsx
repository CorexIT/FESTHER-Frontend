...
export default function AccommodationSection() {
  ...
  const wrapIndex = (i: number) => (i + rooms.length) % rooms.length;
  ...
  const previous = () => setIndex((v) => wrapIndex(v - 1));
  const next = () => setIndex((v) => wrapIndex(v + 1));

  return (
    <section className="acc-section">
      <div className="acc-head">
        <motion.h2 className="acc-title" initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.6 }} transition={{ duration: 0.8, ease: "easeOut" }}>Accommodation</motion.h2>
        <a className="acc-viewall" href="#stay">View All</a>
      </div>

      <div className="acc-viewport">
        <motion.div ref={trackRef} className="acc-track" style={{ x }} drag="x" dragConstraints={{ left: -step * (rooms.length - 1), right: 0 }} dragElastic={0.06} dragMomentum={false} onDragEnd={() => { if (!step) return; setIndex(Math.max(0, Math.min(rooms.length - 1, Math.round(-x.get() / step)))); }}>
          {rooms.map((room) => (
            <article className="acc-slide" key={room.title}>
              ...
              <div className="acc-card">
                <h3 className="acc-room-title">{room.title}</h3><p className="acc-room-size">{room.size}</p><p className="acc-room-desc">{room.desc}</p>
                <div className="acc-actions"><a className="acc-action" href="#stay">Explore</a><span className="acc-actions-sep" aria-hidden="true" /><a className="acc-action" href="#booking">Check Availability</a></div>
                <div className="acc-divider" />
                <ul className="acc-amenities">{room.amenities.map(([key, label]) => <li key={label}>{Icons[key]}{label}</li>)}</ul>
              </div>
            </article>
          ))}
        </motion.div>
      ...
      <div className="acc-nav">
        <button type="button" className="acc-arrow" onClick={previous} aria-label="Previous accommodation">←</button>
        <span className="acc-counter" aria-live="polite">{index + 1} / {rooms.length}</span>
        <button type="button" className="acc-arrow" onClick={next} aria-label="Next accommodation">→</button>
      </div>
    </section>
  );
}
